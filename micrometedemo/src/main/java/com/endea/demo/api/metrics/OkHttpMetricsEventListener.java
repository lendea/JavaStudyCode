...
/**
 * Copied from {@link io.micrometer.core.instrument.binder.okhttp3.OkHttpMetricsEventListener}
 * We need to customize Timer options like histogram.
 * It has a default name and some custom tags(scope, client.type), you have to specify a custom name in builder()
 * if you needs other tags. See https://github.com/micrometer-metrics/micrometer/issues/877.
 */
@NonNullApi
@NonNullFields
public class OkHttpMetricsEventListener extends EventListener {
    ...
    private static final String DEFAULT_METER_NAME = "http.client";
    ...
    protected OkHttpMetricsEventListener(MeterRegistry registry, String requestsMetricName, Function<Request, String> urlMapper,
        ...
        this(registry, requestsMetricName, urlMapper, extraTags, contextSpecificTags, emptyList(), true, false, new Duration[]{});
    }
    ...
    OkHttpMetricsEventListener(MeterRegistry registry, String requestsMetricName, Function<Request, String> urlMapper,
                               ...
                               boolean includeHostTag, boolean publishPercentileHistogram, Duration[] sla) {
    ...
    public static Builder builder(MeterRegistry registry) {
        return new Builder(registry, DEFAULT_METER_NAME);
    }
    ...
    void time(CallState state) {
        ...
        Iterable<Tag> tags = Tags.of(
                        ...
                        // Set default for TSF custom tags to prevent error https://github.com/micrometer-metrics/micrometer/issues/877
                        "client.type", "", "scope", ""
                )
                .and(extraTags)
        ...
        Timer.Builder builder = Timer.builder(this.requestsMetricName)
                // based on
                // https://github.com/micrometer-metrics/micrometer/blob/main/implementations/micrometer-registry-prometheus/src/main/java/io/micrometer/prometheus/PrometheusTimer.java#L56
                // https://github.com/micrometer-metrics/micrometer/blob/e35e4469fdc9145146a9673bff4c73beaa8e920e/micrometer-core/src/main/java/io/micrometer/core/instrument/distribution/TimeWindowFixedBoundaryHistogram.java#L39
                // publishPercentileHistogram creates 60+ buckets, which is too many
                // so prefer sla(fixedBuckets in Builder)
                .publishPercentileHistogram(publishPercentileHistogram)
                .tags(tags)
                .description("Timer of http client operation");
        if (!ArrayUtils.isEmpty(sla)) {
    ...
    private String getUriTag(CallState state, @Nullable Request request) {
        ...
        return state.response == null ? "none" : urlMapper.apply(request);
    ...
    public static class Builder {
        ...
        private boolean includeHostTag = false;
        private Iterable<String> requestTagKeys = Collections.emptyList();
        private boolean generateBuckets = false;
        private Duration[] fixedBuckets = new Duration[]{};
        // Refer https://github.com/grpc-ecosystem/go-grpc-prometheus#histograms
        private Duration[] DEFAULT_BUCKETS = new Duration[]{Duration.ofMillis(5), Duration.ofMillis(10), Duration.ofMillis(25), Duration.ofMillis(50), Duration.ofMillis(100),
                Duration.ofMillis(250), Duration.ofMillis(500), Duration.ofSeconds(1), Duration.ofMillis(2500), Duration.ofSeconds(5),
                Duration.ofSeconds(10), Duration.ofSeconds(25)};
        ...
        /**
         * Historically, OkHttp Metrics provided by {@link io.micrometer.core.instrument.binder.okhttp3.OkHttpMetricsEventListener} included a
        ...
        public OkHttpMetricsEventListener build() {
            return new OkHttpMetricsEventListener(registry, name, uriMapper, tags, contextSpecificTags, requestTagKeys, includeHostTag, generateBuckets, fixedBuckets);
        }
        ...
        /**
         * Sets generateBuckets.
         * Will generate 60+ buckets, see https://micrometer.io/docs/concepts#_histograms_and_percentiles
         * @param generateBuckets
         * @return
         */
        public Builder enableHistogramAndUseGeneratedBuckets(boolean generateBuckets) {
            this.generateBuckets = generateBuckets;
            return this;
        }
         ...
         * @return
         */
        public Builder enableHistogramAndUseFixedBuckets(Duration... fixedBuckets) {
            this.fixedBuckets = fixedBuckets;
            return this;
        }

        public Builder enableHistogramAndUseFixedBuckets() {
            this.fixedBuckets = DEFAULT_BUCKETS;
            return this;
        }
    }
}
